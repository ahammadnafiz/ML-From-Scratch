...
class NeuralNetwork:
    def __init__(self, input_size, hidden_layers, output_size, learning_rate=0.01, 
                 lambda_reg=0.01, keep_prob=0.8, epsilon=1e-8):
        ...
        # Small constant to prevent division by zero in AdaGrad
        ...
        self.accumulated_grads = self.initialize_accumulated_grads()

    def initialize_accumulated_grads(self):
        # Initialize accumulated squared gradients for AdaGrad
        accumulated_grads = {}
        for l in range(1, len(self.layer_dims)):
            accumulated_grads[f'G_W{l}'] = np.zeros_like(self.parameters[f'W{l}'])
            accumulated_grads[f'G_b{l}'] = np.zeros_like(self.parameters[f'b{l}'])
    ...
    def update_parameters(self, grads):
        # Update all weights and biases using AdaGrad optimization
        for l in range(1, self.L + 2):  # +2 because we include output layer
            # Accumulate squared gradients (G_t = G_{t-1} + g_t ⊙ g_t)
            self.accumulated_grads[f'G_W{l}'] += np.square(grads[f'dW{l}'])
            self.accumulated_grads[f'G_b{l}'] += np.square(grads[f'db{l}'])
            
            # Update parameters using AdaGrad formula
            # θ_{t+1} = θ_t - α / sqrt(G_t + ε) ⊙ g_t
            self.parameters[f'W{l}'] -= self.learning_rate * grads[f'dW{l}'] / (np.sqrt(self.accumulated_grads[f'G_W{l}']) + self.epsilon)
            self.parameters[f'b{l}'] -= self.learning_rate * grads[f'db{l}'] / (np.sqrt(self.accumulated_grads[f'G_b{l}']) + self.epsilon)
    ...
    def train(self, X_train, Y_train, epochs=1000, batch_size=32, print_loss=True, save_loss_graph=True):
        ...
        for epoch in range(epochs):
            ...
            for mini_batch in mini_batches:
                ...
                # Step 4: Update parameters using AdaGrad
        ...
        if print_loss or save_loss_graph:
            ...
            plt.title(f'Loss vs. Epochs (Batch Size: {batch_size}, AdaGrad)')
            plt.xlabel('Epochs')
            plt.ylabel('Loss')
            plt.grid(True)
            ...
            if save_loss_graph:
                ...
                plt.savefig(f'plots/loss_curve_adagrad_batch_{batch_size}.png')
                print(f"Loss graph saved to plots/loss_curve_adagrad_batch_{batch_size}.png")
...
def plot_decision_boundary(X, y, model, feature_names):
    ...
    plt.title("Decision Boundary with AdaGrad Optimization")
    plt.colorbar()
    plt.show()
    
def main():
    print("Neural Network with AdaGrad and Regularization")
    print("=" * 50)
    
    # Choose dataset
    dataset_name = "wine"  # Options: "iris", "wine", "breast_cancer"
    ...
    epsilon = 1e-8     # Small constant for numerical stability in AdaGrad
    
    # Create neural network
    nn = NeuralNetwork(input_size, hidden_layers, output_size, learning_rate, lambda_reg, keep_prob, epsilon)
    
    # Train the network
    print("\nTraining neural network with AdaGrad optimization...")
