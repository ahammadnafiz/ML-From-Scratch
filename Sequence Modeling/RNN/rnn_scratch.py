...
W_hh = (
    np.random.randn(n_h, n_h) * 0.01
)  # Hidden to hidden layer weights (recurrent connections)
...
def generate_text(seed_text, length=100, temperature=1.0):
    ...
    for i in range(length):
        ...
        if len(generated) > 0:
            ...
            context_length = min(
                10, len(generated)
            )  # Use last 10 characters as context
            ...
            if (
                i % 50 == 0 and i > 0
            ):  # Every 50 characters, increase diversity slightly
...
def generate_long_text_with_breaks(
    seed_text, length=1000, temperature=1.0, break_interval=200
):
    """
    ...
    while remaining_length > 0:
        ...
        # Generate a chunk
        chunk = generate_text(
            generated[-50:], length=chunk_length, temperature=temperature
        )
