...
class NeuralNetwork:
    def __init__(self, input_size, hidden_layers, output_size, learning_rate=0.01, 
                 lambda_reg=0.01, keep_prob=0.8, beta=0.9, epsilon=1e-8):
        ...
        self.beta = beta           # Decay rate for squared gradients
        ...
        # Initialize squared gradients cache for RMSprop
        self.squared_gradients_cache = self.initialize_squared_gradients_cache()

    def initialize_squared_gradients_cache(self):
        # Initialize the cache for RMSprop
        squared_gradients = {}
        for l in range(1, len(self.layer_dims)):
            squared_gradients[f'dW{l}'] = np.zeros_like(self.parameters[f'W{l}'])
            squared_gradients[f'db{l}'] = np.zeros_like(self.parameters[f'b{l}'])
    ...
    def update_parameters(self, grads):
        # Update all weights and biases using RMSprop
        for l in range(1, self.L + 2):  # +2 because we include output layer
            # Update squared gradients cache
            self.squared_gradients_cache[f'dW{l}'] = self.beta * self.squared_gradients_cache[f'dW{l}'] + (1 - self.beta) * np.square(grads[f'dW{l}'])
            self.squared_gradients_cache[f'db{l}'] = self.beta * self.squared_gradients_cache[f'db{l}'] + (1 - self.beta) * np.square(grads[f'db{l}'])
            
            # Update parameters using RMSprop formula
            self.parameters[f'W{l}'] -= self.learning_rate * grads[f'dW{l}'] / (np.sqrt(self.squared_gradients_cache[f'dW{l}']) + self.epsilon)
            self.parameters[f'b{l}'] -= self.learning_rate * grads[f'db{l}'] / (np.sqrt(self.squared_gradients_cache[f'db{l}']) + self.epsilon)
...
def main():
    ...
    beta = 0.9         # RMSprop decay rate parameter
    epsilon = 1e-8     # Small constant to avoid division by zero
    
    # Create neural network
    nn = NeuralNetwork(input_size, hidden_layers, output_size, learning_rate, lambda_reg, keep_prob, beta, epsilon)
